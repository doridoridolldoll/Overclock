...
export default {
  ...
  setup(){
    ...
    axios.post(url, { page:page, type:"", category:"perietc" }, { headers })
    .then(function(res){
  ...
  axios.post(url, { page: 1, category: "perietc" }, { headers })
            .then(function (res) {
            console.log(res);
            state.dtoList = res.data.dtoList,
                state.end = res.data.end,
  ...
  function Join(list,i){
    ...
    store.commit("setPrice", ...[state.price[i]]);
      ...
      axios.get(url2, {page: 1, category: "perietc" }, { headers2 }).then(function(){
