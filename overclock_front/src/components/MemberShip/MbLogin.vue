  ...
  <div id="back" class="d-flex flex-column form-floating" action="login" method="post">
    <form class="validation-form" @submit.prevent>
      ...
      <div class="input-group idsection px-5 my-2">
        <input class="form-control" type="text" name="email" id="email" ref="email" v-model="state.form.email"
          placeholder="이메일" required autofocus>
      </div>
      <div class="input-group passwordsection px-5 my-2 w-100">
        <input class="form-control" type="password" name="password" id="password" ref="email"
          v-model="state.form.password" placeholder="비밀번호" required autofocus>
      </div>
      <div class="px-5 my-4">
